...
class OriginalAO(Optimizer):
    ...
    def SCF(self,epoch):
        ...
        SCF_val = 2 * exp(1 - (( epoch *(0.01*epoch))/self.epoch))*Dir
        return SCF_val
        ...
        for idx in range(0, self.pop_size):
            ...
            if (epoch + 1) <= (2 / 3) * self.epoch:  # Eq. 3, 4
                if np.random.rand() < 0.5:
                    ...
                    idx_r = np.random.choice(list(set(range(0, self.pop_size)) - {idx}))
                    #pos_new = self.g_best[self.ID_POS] * levy_step + self.pop[idx][self.ID_POS] + np.random.rand() * (y - x)  # Eq. 5
                    pos_new= self.pop[idx_r][self.ID_POS]+(self.SCF(epoch))*((self.g_best[self.ID_POS])-(self.pop[idx][self.ID_POS]))*np.random.rand() * (y - x)     # NEWLY ADDED
            else:
                if np.random.rand() < 0.5:
                   ...
                   idx_r = np.random.choice(list(set(range(0, self.pop_size)) - {idx}))
                   pos_new = (self.problem.lb) +(np.random.rand() * (self.problem.ub - self.problem.lb))+ np.random.rand()*((self.pop[idx_r][self.ID_POS])-(self.g_best[self.ID_POS])) * \
                             (self.SCF(epoch))*(1-(epoch/self.epoch)) #newly added line
                else:
                    pos_new = QF * self.g_best[self.ID_POS] - (g1 * self.pop[idx][self.ID_POS] * np.random.rand()) - \
